...
function popupHtml(pin: Pin): string {
  const name = escapeHtml(pin.place.name);
  const address = pin.place.address ? `<p class="map-popup-detail">${escapeHtml(pin.place.address)}</p>` : '';
  return `<p class="map-popup-day">Day ${pin.dayIndex}</p><p class="map-popup-name">${name}</p>${address}`;
}
