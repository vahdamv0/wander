    ...
    doLast {
        ...
        out.writeText(
            buildString {
                ...
                appendLine()
                appendLine("Two sections here, because the jar has two dependency trees: the browser")
                appendLine("bundle under META-INF/resources, and the server's jars under BOOT-INF/lib.")
                appendLine("Build-time tooling appears in neither — it is not distributed.")
                appendLine()
                appendLine("The container image carries a **third** part, appended to this file when the")
                appendLine("image is built: the operating-system packages, which include the KItinerary,")
                appendLine("Qt, poppler and ZXing stack behind booking import. It cannot be generated")
                appendLine("here, because the package set belongs to the runtime stage rather than to")
                appendLine("the source tree and differs between amd64 and arm64. So this file covers the")
                appendLine("jar; /app/THIRD-PARTY.txt inside the image covers the image. See")
                appendLine("deploy/os-notices.sh.")
                appendLine()
